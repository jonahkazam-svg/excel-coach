...
function Invoke-XlAction($req){
  ...
  # SAFETY BASELINE for auto-revert: how many hard formula errors exist BEFORE we touch anything, and
  # how deep the undo stack is now. If our edit increases the error count, we peel back everything we
  # pushed (the write + any verify repairs) so the sheet is NEVER left broken. (#DIV/0! is excluded -
  # that's the empty-template placeholder, not breakage.)
  $errBefore=0; try{ $errBefore=([regex]::Matches([string]$fresh,'#NAME\?|#REF!|#VALUE!|#NUM!')).Count }catch{}
  $stackBefore=0; try{ if($sync.undoStack){ $stackBefore=[int]$sync.undoStack.Count } }catch{}
  ...
  if((-not $aops) -or ($aops -match '^\s*NOTACTION') -or ($aops -notmatch '(?m)^(SET|PUT|SHEET|FMT)\s')){
    ...
    $frr=& curl.exe -s --max-time 240 "https://api.openai.com/v1/chat/completions" -H ("Authorization: Bearer "+$sync.key) -H "Content-Type: application/json" -d ("@"+$fbf)
  ...
  if($applied -and $r -and ($r -notmatch '^(Excel is not open|No active workbook|Excel would not let me in)')){
    ...
    for($vround=1;$vround -le 2;$vround++){   # 2 verify rounds: correctness > speed - catch and repair anything that didn't land or is wrong
      ...
      $vrr=& curl.exe -s --max-time 180 "https://api.openai.com/v1/chat/completions" -H ("Authorization: Bearer "+$sync.key) -H "Content-Type: application/json" -d ("@"+$vbf)
  ...
  # AUTO-REVERT SAFETY NET: if this edit (write + any repairs) INTRODUCED hard formula errors, undo
  # the whole thing so the model is never left destroyed. Peels back exactly the snapshots we pushed.
  if($applied){
    try{
      $finalTxt=$null; try{ $finalTxt=Read-ExcelLive 0 1200 }catch{}
      if($finalTxt){
        $errAfter=([regex]::Matches([string]$finalTxt,'#NAME\?|#REF!|#VALUE!|#NUM!')).Count
        if($errAfter -gt $errBefore){
          $peel=0; try{ $peel=[int]$sync.undoStack.Count - $stackBefore }catch{}
          if($peel -lt 1){ $peel=1 }
          for($u=0;$u -lt $peel;$u++){ if(Get-Command Undo-XlOps -ErrorAction SilentlyContinue){ try{ Undo-XlOps | Out-Null }catch{} } }
          try{ [IO.File]::AppendAllText(($env:TEMP+"\xc_hands.log"),((Get-Date).ToString("HH:mm:ss")+"  AUTO-REVERT: edit added "+($errAfter-$errBefore)+" formula error(s) -> undone ("+$peel+" snapshot(s))`r`n`r`n"),(New-Object System.Text.UTF8Encoding($false))) }catch{}
          $r="I held off - that would have broken "+($errAfter-$errBefore)+" formula(s) on your sheet, so I undid it to keep your model safe. Try one section at a time (e.g. 'fill the cash flow statement'), or tell me the exact cells and what they should be."
        }
      }
    }catch{}
  }
